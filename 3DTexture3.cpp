	...
	if (tri_type == TRIANGLE_TOP || tri_type == TRIANGLE_BOTTOM) //平顶三角形
	{
			...
			|| (x2 < min_clip_x) || (x2 > max_clip_x))
		{
			for (int yi = ys; yi <= ye; yi++, dest_addr += mempitch)
			{
				...
				{
					int sp = si >> FIXP16_SHIFT;
					int tp = ti >> FIXP16_SHIFT;
					int sn = sp + 1;
					int tn = tp + 1;
					if (sn >= BMP_SIZE) { sn = BMP_SIZE - 1; }
					if (tn >= BMP_SIZE) { tn = BMP_SIZE - 1; }
					//取相邻的像素点
					int texteltpsp = ((UINT*)bitmap->buffer)[sp*bitmap->bitmapinfoheader.biWidth + tp];
					int texteltnsp = ((UINT*)bitmap->buffer)[sp*bitmap->bitmapinfoheader.biWidth + tn];
					int texteltpsn = ((UINT*)bitmap->buffer)[sn*bitmap->bitmapinfoheader.biWidth + tp];
					int texteltnsn = ((UINT*)bitmap->buffer)[sn*bitmap->bitmapinfoheader.biWidth + tn];
					//取纹素

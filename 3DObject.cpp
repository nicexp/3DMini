...
void LightObject2ByFlat(OBJECT4DV2_PTR obj, CAM4DV1_PTR cam, LIGHTV1_PTR lights, int max_lights)
{
	...
	for (int poly = 0; poly < obj->num_polys; poly++)
	{
		...
		ver1 = cur_poly[poly].vert[0];
		ver2 = cur_poly[poly].vert[1];
		ver3 = cur_poly[poly].vert[3];
	...
	for (int poly = 0; poly < obj->num_polys; poly++)
	{
		...
		for (int cur_light = 0; cur_light < max_lights; cur_light++)
		{
			...
			else if (lights[cur_light].attr & LIGHTV1_ATTR_SPOTLIGHT2)
			{
				for (int ver = 0; ver < 3; ver++)
				{
					dp = VECTOR4D_DOT(&obj->vlist_trans[cur_poly->vert[0]].n, &lights[cur_light].dir);
					if (dp>0)
					{
						//顶点到光源向量
